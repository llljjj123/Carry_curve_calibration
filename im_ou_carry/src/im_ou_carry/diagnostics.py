...
def model_metrics(fits: pd.DataFrame, split_date: object) -> pd.DataFrame:
    ...
    for sample, mask, carry_col, price_col in (
        ...
        rows.append({"model": "ou_one_factor", "sample": sample, **_error_metrics(fits.loc[mask, carry_col], fits.loc[mask, price_col])})
