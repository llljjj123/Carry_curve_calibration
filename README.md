...
其中$B(\kappa,\tau)=(1-e^{-\kappa\tau})/(\kappa\tau)$。当快慢因子的符号相反时，两种衰减速度的叠加可以产生U形或倒U形，因此比单$\kappa$模型具有更强的期限结构拟合能力。当前版本假设快慢因子的随机冲击相互独立，在上述计算中对所有期限使用同一个观测误差波动率。
...
代码同时给出path-wise derivate和bump-and-value结果以及二者的绝对差。它们分别代表慢因子和快因子的方向性对冲比率，并不是由一张期货合约唯一确定的同一个delta，故我们只能在一个时间要么对冲慢因子或快因子，无法做到同时对冲二者。


# 演示
对应`Demo`，演示了一种最简单的假设业务情况。即客户于一个初始给定日期买入一笔期货（为简单暂定为中金所期货，代码中使用IM2612）和一个上述美式看跌，两者到期日一致。同时在代码中给定一个用于拟合快慢$\kappa$参数的时间窗口并选择$q$方法或$F$方法，程序会自动开始拟合，并汇报拟合结果。最后代码会用拟合结果对上述美式看跌进行定价，此价格即为我们（卖方）在入场交易时应当对客户额外收取的指数点（如定价为$50$，则对应一笔IM，应当收取$50\times 200 = 10000$元）。同时代码也会得到两个delta，$\Delta_j^F$，即对冲慢因子或快因子所需delta
...
4. 目前只做了对应一个期货的情况，实际业务中可能存在期货滚动开仓的情况，依据真实业务情况再对需定价期权进行相应改动和处理。
5. 对于delta对冲，因为模型选择原因存在两个delta，$\Delta_j^F$。这里我认为要么选择一个更简单的单因子模型，从而存在唯一delta；要么通过回测/根据到期期限长短/入场时分红率$q$是否存在较明显mean-reverting趋势（比如正常是$10\%$，现在是$25\%$）等等手段来确定使用快或慢因子；或者直接取平均值。总之这里也需要进一步观察、研究。
