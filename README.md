...
$$
\log\left(F_{t,T}/S_t\right) -r\tau = -\theta \tau -A_s(\tau)x_{s,t} - A_f(\tau)x_{f,t} + u_{t,T}
$$
...
# 演示
对应`Demo`，演示了一种最简单的假设业务情况。即客户于一个初始给定日期买入一笔期货（为简单暂定为中金所期货，代码中使用IM2612）和一个上述美式看跌，两者到期日一致。同时在代码中给定一个用于拟合快慢$\kappa$参数的时间窗口并选择$q$方法或$F$方法，程序会自动开始拟合，并汇报拟合结果。最后代码会用拟合结果对上述美式看跌进行定价，此价格即为我们（卖方）在入场交易时应当对客户额外收取的指数点（如定价为$50$，则对应一笔IM，应当收取$50\times 200 = 10000$元）。同时对于单期货对冲代码也会得到两个delta，$\Delta_j^F$，即对冲慢因子或快因子所需delta；对于双期货对冲，代码会得到对应期货所需对冲delta。

# 下一步的工作

1. 此项目仅使用了Vasicek模型作为模型初探，后续可考虑使用更复杂一些的模型如CIR等，进一步观察拟合效果。
2. 模型拟合使用的交易数据量也可进行探讨，应该使用更长期的以获得更稳定的模拟效果，还是使用相对较短期的来更好地捕捉现在的情况。
3. 对于得出的期权价值也可进行回测，来检验在大数定理下期权费能否平均地覆盖掉卖出期权造成的赔付。
4. 目前只做了期权对应一个标的期货的情况，实际业务中可能存在期货滚动开仓的情况，依据真实业务情况再对需定价期权进行相应改动和处理。
5. 对于delta对冲，模型计算了两种delta：1. 使用对应标的单期货对冲delta，此时因分红率$q$由两因子$x_s$和$x_f$共同决定，故使用单期货只能完整对冲1个因子，而对第二个因子通常会留下残余风险，可能表现为部分对冲或过度对冲；2. 使用两期货对冲，此时只要矩阵$G$是满秩的即可locallay对冲两因子$x_s$和$x_f$的一阶风险。双期货对冲看似解决了同时对冲两因子问题，但这是基于不同期限分红率都完全遵守两因子Vasicek模型的假设，这种方式引入了期货间不同期限分红率变化的问题，使整体模型更依赖于底层Vasicek模型假设。而单因子对冲则没有不同期限期货间分红率变化问题，可能对Vasicek模型风险暴露更小，对拟合参数变化的敏感性也更小（当然这也需要进一步通过检测判断）。这两种方式各有优劣，因此下一步可以通过回测的方式来验证：a.不对冲；b.单期货对冲；c.双期货对冲，这三种方式的总P&L、尾部风险、对冲greeks稳定性等方式来进一步决定使用哪种对冲方式。
