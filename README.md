...
# 演示
对应`Demo`，演示了一种最简单的假设业务情况。即客户于一个初始给定日期买入一笔期货（为简单暂定为中金所期货，代码中使用IM2612）和一个上述美式看跌，两者到期日一致。同时在代码中给定一个用于拟合快慢$\kappa$参数的时间窗口，程序会自动开始拟合，并汇报拟合结果。最后代码会用拟合结果对上述美式看跌进行定价，此价格即为我们（卖方）在入场交易时应当对客户额外收取的指数点（如定价为$50$，则对应一笔IM，应当收取$50\times 200 = 10000$元）。同时代码也会得到两个delta，$\Delta_j^F$，即对冲慢因子或快因子所需delta

# 下一步的工作

1. 此项目仅使用了Vasicek模型作为模型初探，后续可考虑使用更复杂一些的模型如CIR等，进一步观察拟合效果。
2. 模型拟合过程中的数据并没有进行过任何二次加工，由于很短期的期货合约分红率受成交噪声影响很大，而快$\kappa$的拟合又极依赖于短期期货合约，故对于其的二次处理可能对拟合效果造成较大影响。我们初步可以先为短期合约设置一个分红率上下限（可为固定，也可基于次短期分红率浮动），来先排除掉异常值。
3. 模型拟合使用的交易数据量也可进行探讨，应该使用更长期的以获得更稳定的模拟效果，还是使用相对较短期的来更好地捕捉现在的情况。
4. 目前的快慢$\kappa$模型对于在使用不同期限数据进行拟合时常发生$\kappa_f - \kappa_s$，即快慢$\kappa$之差达到拟合上边界的情况。初步的做法可以是直接放宽上边界，但是我更倾向于先对原始数据进行加工和清理之后，观察一下新的拟合情况再做进一步决定。
5. 对于得出的期权价值也可进行回测，来检验在大数定理下期权费能否平均地覆盖掉卖出期权造成的赔付。
6. 目前只做了对应一个期货的情况，实际业务中可能存在期货滚动开仓的情况，依据真实业务情况再对需定价期权进行相应改动和处理。
7. 对于delta对冲，因为模型选择原因存在两个delta，$\Delta_j^F$。这里我认为要么选择一个更简单的单因子模型，从而存在唯一delta；要么通过回测/根据到期期限长短/入场时分红率$q$是否存在较明显mean-reverting趋势（比如正常是$10\%$，现在是$25\%$）等等手段来确定使用快或慢因子；或者直接取平均值。总之这里也需要进一步观察、研究。
